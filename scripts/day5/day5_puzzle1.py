...
data = pd.read_csv("input/day5.csv", header=None, usecols=[0, 1, 2, 3])
data = data.rename({0: "x1", 1: "y1", 2: "x2", 3: "y2"}, axis="columns")

vert = data[data["x1"] == data["x2"]]
vert["diff"] = vert["y2"] - vert["y1"]
diff_list = list(vert["diff"].values)

for i in range(len(diff_list)):
    diff_list[i] = list(range(diff_list[i]+1))
diff_list_series = pd.Series(diff_list, index=vert.index)
vert["diff_list"] = diff_list_series
vert = vert.explode("diff_list")
print(vert)
