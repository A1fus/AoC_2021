...
calls = pd.read_csv("input/day4/calls.csv", header=None, usecols=[0])
boards = pd.read_csv("input/day4/boards.csv", header=None, usecols=[0, 1, 2, 3, 4])
boards["board_n"] = (boards.index / 5).astype(int)
boards = boards.set_index(["board_n", boards.index])
boards_marked = boards.copy(deep=True)
print(boards)
broken = False
for i in range(calls.shape[0]):
    boards_marked = boards_marked.replace(calls[0][i], np.nan)
    # Check cols
    for j in [0, 1, 2, 3, 4]:
        cols = boards_marked[boards_marked[j].isnull()].index.tolist()
        cols_list = [i[0] for i in cols]
        counts = Counter(cols_list)
        if counts.most_common(1)[0][1] == 5:
            winning_board = counts.most_common(1)[0][0]
            broken = True
            break
    if broken:
        break
    # Check rows
    rows = boards_marked[[0, 1, 2, 3, 4]].isnull().apply(lambda x: all(x), axis=1)
    if len(rows[rows].index.tolist()) == 1:
        winning_board = rows[rows].index.tolist()[0][0]
        break

print((boards_marked.loc[winning_board].sum(axis=0, skipna=True).sum()) * winning_board)
