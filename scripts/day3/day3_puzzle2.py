...
co_data = data
for i in range(12):
    data_counts = ox_data.apply(pd.Series.value_counts)
    if data_counts[i].idxmax() == data_counts[i].idxmin():
        ox_data = ox_data.loc[data[i] == 1]
    else:
        ox_data = ox_data.loc[data[i] == data_counts[i].idxmax()]

    if len(ox_data) == 1:
        ox = ox_data.to_numpy()
        break

ox_int = int("".join(str(x) for x in ox[0]), 2)

for i in range(12):
    data_counts = co_data.apply(pd.Series.value_counts)
    if data_counts[i].idxmax() == data_counts[i].idxmin():
        co_data = co_data.loc[data[i] == 0]
    else:
        co_data = co_data.loc[data[i] == data_counts[i].idxmin()]

    if len(co_data) == 1:
        co = co_data.to_numpy()
        break
co_int = int("".join(str(x) for x in co[0]), 2)
print(ox_int, co_int, ox_int * co_int)

"""TEST ON EXAMPLE DATA"""
data = pd.read_csv(
    "input/day3_ex.csv", header=None, usecols=[0, 1, 2, 3, 4]
)
ox_data = data
co_data = data
