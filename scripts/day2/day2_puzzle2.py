...
"""
data = pd.read_csv("input/day2.csv", header=None, usecols=[0, 1])
data = data.rename({0: "direction", 1: "distance"}, axis="columns")
aim = 0
pos = 0
depth = 0
for i in range(data.count()[0]):
    if data.iloc[i]["direction"] == "forward":
        pos = pos +data.iloc[i]["distance"]
        depth = depth + (aim * data.iloc[i]["distance"])
    elif data.iloc[i]["direction"] == "up":
        aim = aim - data.iloc[i]["distance"]
    elif data.iloc[i]["direction"] == "down":
        aim = aim + data.iloc[i]["distance"]
